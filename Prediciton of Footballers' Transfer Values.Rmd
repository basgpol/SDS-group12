...
To extract the relevant data we built three web scrapers. One for Transfermarkt, another for Google and a third for Wikipedia. The scraper for Transfermarkt both collected all transfer information from the transfer windows "Summer 15" and "Winter 16" in the five leagues and the individual performance statistics from each transferred player. We then used the Google-scraper to gather information on the number of google hits when searching for the name of each transferred player followed by the word "footballer". We use google hits as a proxy for a players popularity and because the variable probably correlates attributes which are not measured in the performance statistics. Information on transfer, performance statistics and google hits were then merged into one data frame consisting of player data.
The scraper for Wikipedia ran through the different webpages with the table ranking for the season 14/15 in the five leagues. The table rankings were then merged into one data frame containing the club data.
The two different datasets were at first cleaned and then merged into one big dataset containing only the variables thought to be relevant in predicting transfer values.  
...
## Data Cleaning
In order to do proper visualisations and predictions we have to clean the raw data. Our experience was that Transfermarkt was inconsistent in the way of handling statistics which made the cleaning process challenging.
...
Thes first visual (figure X) below is a mapping of all the clubs buying players with the size of the plots corresponding the amount the their transfer spending. The map allows us to check the validity of our sample, by comparing it to the most well known clubs on the football market. Unsurprisingly, the Premier League appeared to have on average more clubs with a higher spending, and the most reknown clubs such as PSG, Real Madrid or Barcelona clearly appeared on the map.

&nbsp;

```{r, echo=FALSE, message = FALSE, fig.pos="H", fig.width=3.5, fig.height=3.5, fig.cap="Maps of main European Clubs"}
...
Our second visualisation focus on the transfer path of each transfered player in the dataset. This highlights the fact that the market for football players is a European market, or even a global one and that barriers between countries don't seem to play a big role in transfering players. As you can see on the map, most of the players are traded between countries and not within them.
...
## Age and contract length seem to have influence on transfer value  

The following two scatterplots (figure X) indicate that age and period left on contract correlates with a player's transfervalue. The scatterplot to the left shows the relationship between age and transfer value. The observations indicate a bell-shaped relationship with a flat top around 25 years. Perhaps forseeably, most players are traded while in their prime, between 20 and 27 years old. One interpretation might be that the promises of a rising star are more sought after than the celebrity of an older player. Due to bell-shaped relationsship we include age squared as a predictor in the later models.  
The scatterplot to the right highlights a correlation between the time left of a player's contract and his transfer value. As you could expect, the more time left on a contract, the higher a price for a player will be. When the period is less than 20 months, the transfer fees do not exceed 30 million pounds. When more than 20 months, the transfer fees can go up to 30 million pounds. Some transfer fees even reach 40 million pounds, when the time left on the contract exceeds 40 months.
...
The following two bar charts (figure X) show the relationship between the transfer value and club status of the buyer (left) and transfer value and league of the buying club (right) respectively.
The graph to the left clearly shows, that clubs from the Premier League spend much more money on each player compared the clubs in all the other leagues. The league of the buying club could therefore be a good indicator of a players transfervalue. A reason for this pattern could be Premier League's popularity all over the world, resulting in higher TV licensing rights, bigger sales of fan merchandise and a higher attendance during british match. Both being able to buy expensive players and the ambition to remain at the center of international football push british clubs into buying more expensive players (also shown on the map in figure X).  
The graph to the right shows that the more successful a club is, the more money it can afford to spend on transfers. One interesting feature of this graph however is that the promoted clubs are spending a bit more money on transfer than the bottom clubs. This can be explained by their intention to improve their roster in order to be able to compete in the major league. This graph therefore indicates that the ranking of the buying club in the league last season can be used as a signal for the transfer value of a player.  
...
In the appendix we also have a bar chart of the average spending on transfers per club in the different leagues. The printed visualisations in the papers are supporting that we include this variables in the upcomming prediction models. Graphs for most of the reamning predicting variables are avaiable in the R code.   
...
As the second prediction model we use the linear regression model - ordinary least square (OLS). OLS works by minimizing the *sum of squared residuals* (SSR) and is thereby also minimizing the bias. Unfortunately, the goal of minimizing the in-sample errors lead to a risk of overfitting and thereby increase the errors due to variance. Therefore, OLS is considered among the best prediction methods. On the other hand, OLS is a very good method when inference is the goal because the model's significance can be tested. When applying the OLS model on the test sample we receive a RMSE of 6.34.
...
$|T|$ is the number of terminal nodes in the subtree $T$, $R_m$ is the subspace of the region M, and $y_{R_m}$ is the predicted response associated with $R_m$. When pruning the tree, we obtain different version of the tree as a function of $\alpha$. We evaluate the prediction power of each version of the tree using *K-fold cross-validation* (in our case we divide the training data into 10 folds) to choose the right $\alpha$ value.
...
## Random Forest
As described, the decision tree often has issues with overfitting which creates errors due to variance. The issues can be reduced by another prediction method called *random forest* (RF). The RF is a so-called ensemple model which means that the model consists of several smaller decision trees (in this case regression trees). RF has great similarities with the concept of *bagging* or *bootstrap aggregating* (Breiman, 1996) where the core idea is to reduce the error due to variance from one model by building several models and use their average prediction. A random forest prediction can by divided into 4 steps:  
...
We can also use the RF to calculate and rank the importance of each predictor in the data set. This is done by calculating *%IncMSE*. The %IncMSE states the percentage increase of the mean squared error (estimated with the out-of-bag error rate) if the particular variable is permuted (which mean that the values are randomly shuffled). The higher the number the more important is the variable. As with the decision tree google search results is shown to be the most important variable[^3]. 
...
To select the best predictors out of all 25 variables in the data set we used several visualisations to search for correlation patterns. Furthermore, we used the Google Maps API to visualize our data sample. We then selected 11 predictors which all seemed to have an influence on the players' transfer value.  
