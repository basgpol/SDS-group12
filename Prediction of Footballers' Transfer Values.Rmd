...
author: 
- Bjørn Skeel-Gjørling, Chritian Lund Sørensen, 
- Guillaume Slizewicz & Amer Skaljic  
- Group 12
- 28731 Characters
date: "26 August 2016"
output: 
...
### The Final Dataset
The final and tidy dataset contains 369 observations and 25 variables. All the variables are listed with a short description in the appendix. As stated, the final data set contains a lot of variables. We will therefore in the following section describe how we used different visualisations to explore and select the different variables that seemed most suited for predicting transfer values.

# Data Visualisation
The objective of this section is to highlight possible correlations and important variables via the use of data visualisation. The creation of maps, scatter plots and bar charts enabled us to have a better understanding of our dataset and convey its most important characteristic. After experimenting different scatterplots, we were able to have a better grasp of the relationship between transfer fees and other variables such as age, position, appearance, total goals and time left on contract. Due to space limitations we only include visualisations and detailed descriptions for a handful of the predictors used in the later models.[^3] 
In order to transform our dataset into graphs and maps we used different digital tools: GGplot for scatterplots and bar charts and Google maps API for maps. The dataset used for these visualisations was cleaned by removing all observations with a transfer fee equal to zero.

[^3]:More visualisations are available in the Appendix/R code.

## A European Market Dominated by the UK.
Figure 2 below is a mapping of all the clubs buying players. The size of the dots corresponds to the amount of money spent on transfers. The map allows us to check the validity of our sample by comparing it to the most well-known clubs. Unsurprisingly, the Premier League appears to have more clubs with a higher spending. Furthermore, renown clubs such as Real Madrid, Barcelona and PSG clearly appear on the map.

```{r, echo=FALSE, message = FALSE, fig.pos="H", fig.width=4, fig.height=4, fig.cap="The European transfer market"}
...
Our second visualisation (figure 3)[^4] illustrates the transfer path of each transferred player in the dataset. This highlights the fact that the market for football players is a European market, or even a global one and that country borders do not seem to play a major role in transferring players. As you can see on the map, most of the players are traded between countries and not within them.

[^4]:For simplification reasons, transfers from outside Europe are not represented here.

```{r, echo=FALSE, message = FALSE, fig.width=3.5, fig.height=3.5, fig.cap="Maps of transfer paths"}
...
The following two scatterplots (figure 4) indicate that age and period left on contract correlate with a player's transfer value. The scatterplot to the left shows the relationship between age and transfer value. The observations indicate a bell-shaped relationship with a flat top around 25 years. Perhaps foreseeably, players in their prime (between 20 and 27 years old) are traded for higher prices. One interpretation might be that the promises of a rising star are more sought after than the celebrity of an older player. Due to bell-shaped relationship we include age squared as a predictor in the later models.  
The scatterplot to the right highlights a correlation between the time left of a player's contract and his transfer value. As you could expect, the more time left on a contract, the higher the price. When the period is less than 20 months, the transfer fees do not exceed 30 million pounds in our sample. When more than 20 months, the transfer fees go up to 30 million pounds in our sample. Some transfer fees even reach 40 million pounds, when the time left on the contract exceeds 40 months.

![Scatter plots](Exam_project/scatter_plots.jpeg)
...
The first graph clearly shows, that clubs from the Premier League spend much more money on each player compared to clubs from other leagues. The league of the buying club could therefore be a good indicator of a players transfer value. A reason for this pattern could be Premier League's popularity all over the world, resulting in higher TV rights income, bigger sales of fan merchandise and higher attendance for British matches. Both being able to buy expensive players and the ambition to remain at the centre of international football push British clubs into buying more expensive players.  

![Bar Charts](Exam_project/bar_charts.jpeg)
...
In the appendix we also have a bar chart of the average spending on transfers per club in the different leagues. The visualisations support the inclusion of the mentioned variables in the prediction models.   

# Prediction Models for Transfer Value

## Prediction versus causality

The main purpose of this section is to find the best way of predicting transfer values. We therefore focus on finding the most efficient way to estimate our dependent variable when we know the value of several different independent variables - hereafter called predictors. The issue of prediction stands in contrast to the regular goal in social sciences where we want to estimate the causal effect of one particular independent variable on the dependent variable. The key difference is that the focus shifts from the individual effect of a predictor (effects of causes) to the interplay between several predictors and how this interplay can help us predict the value of the dependent variable. Due to this scientific purpose we use the predictors as signals of a specific transfer value and do not pay attention to whether they come before the dependent variable in time or whether they suffer from omitted variable bias. A good example of this is our use of google hits as a predictor for the players' transfer value. The amount of hits is recorded in real time and therefore after the transfer was made. Due to the time order, the recorded number of google hits cannot be the reason why the club bought the particular player for a given price. On the other hand a high number of google hits can signal that the player has had a successful career or was part of a news generating transfer. The Achilles' heel of a predictor recorded after the actual transfer is that it will not accurately predict the future transfer value. Unfortunately, Google has closed down their API and do not allow us to access the number of google hits at a particular day back in time. We therefore chose the far from perfect proxy for popularity that we had at our disposal. In that regard, we have to keep in mind, that the prediction models built in this paper probably overestimate the importance of google search results. For future predictions the problem could be solved by recording search results for a database of players before a transfer window and then compare the data. The final predictors are listed in the appendix.
...
We evaluate and compare the models by using the root mean squared error (RMSE) which is stated in the equation below. The RMSE measures how well the model predicts the observations in the test sample. We created a function in R to calculate the RMSE.
...
It is computationally infeasible to consider all possible splits into $J$ regions. Therefore, an approach called *recursive binary splitting* (do not take into account how a split affects the following splits) is used (Friedman et. al, 2001). In our case the first split is searchresults>50300 which means that this division of the observations provides the greatest reduction of the SSR of all possible first splits across all predictors and all values.  
There is a risk that growing a tree in this way will lead to overfitting. To avoid this, we can use a procedure called *pruning* which punish the complexity of the model. To prune the tree, we use the following equation:
$$ \sum_{m=1}^{|T|}\sum_{x_i \in R_j}(y_i-\hat{y}_{R_j})^2+\alpha|T|  $$
$|T|$ is the number of terminal nodes in the subtree $T$, $R_m$ is the subspace of the region M, and $y_{R_m}$ is the predicted transfer value associated with $R_m$. When pruning the tree, we obtain different versions of the tree as a function of $\alpha$. We evaluate the prediction power of each version of the tree using *K-fold cross-validation* (in our case we divide the training data into 10 folds) to choose the right $\alpha$ value.
A new tree is grown on all but the $10^{th}$ fold of the training data. This is done for each value of $\alpha$. All the grown trees are evaluated by calculating the MSE on the $10^{th}$ fold. The $\alpha$ value with the lowest average MSE across the folds is chosen and then used on the original tree.
As with the Lasso model overfitting was not an issue and the original tree produced a lower RMSE than the pruned one. The RMSE is 6.60.
...
```{r, echo=FALSE, message = FALSE,fig.width=3.5, fig.height=3.5,fig.pos="!H", fig.cap="Average Club Spending Between Leagues"}
